...
REPORTS_GLOB = "reports/runs/**/*.json"  # pick up run report JSONs
...
def load_json_reports(pattern=REPORTS_GLOB):
    ...
    for path in glob.glob(pattern, recursive=True):
        try:
            ...
            # skip unreadable files (corrupt, non-json)
            continue
...
def ingest_all(persist_dir=PERSIST_DIR):
    ...
    # Chroma persistence is handled by the library after creation; print summary
